    ...
    <div class="header">
        <div class="container">
            ...
            <table class="table table-bordered tabulka">
                ...
                <?php
                $order_id_status = Db::queryAll('SELECT order_id, Name, status, payment FROM orders WHERE status != "delivered" && status !="Confirmed" && status !="Cooking"');
                foreach ($order_id_status as $keys1 => $values1) {
                    ...
                    foreach ($tracking as $keys => $values) {
                        ...
                        <tr class="cartRowsHolder">
                            ...
                            <td class="cartRow"><?php echo $values1["Name"]; ?></td>
                            <td class="cartRow"><?php echo $product["name"]; ?></td>
                            <td class="cartRow"><?php echo $values["qty"]; ?></td>
                            <td class="cartRow"><?php echo $values1["payment"]; ?></td>
                            <td class="cartRow"><?php echo $values1["status"]; ?></td>

                            <td class="cartRow">
                            <form method="post" action="mario.php?action=update">
